function plot_peakProfile(SDAT, x, y, eqname, str, unitno, convf, unitstr, nprofile, ncase, pp_bool)
...
for i = 1:neq
    ...
    for j = 1:nprofile
        ...
        for k = 1:ncase
            ...
            ylabel('Depth [m]');
            legendstr{k} = SDAT{k,ncase*j-(ncase-k)}.case;
            title(strcat(eqname{i},':  ',SDAT{k,j*ncase}.profile,' ', '-',' ',str,' Profile, YZ'));
        end
    ...
    for j = 1:nprofile
        ...
        for k = 1:ncase
            ...
            ylabel('Depth [m]');
            legendstr{k} = SDAT{k,ncase*j-(ncase-k)}.case;
            title(strcat(eqname{i},':  ',SDAT{k,j*ncase}.profile,' ', '-',' ',str,' Profile, ZX'));
        end
        legend(legendstr,'Location','SouthEast');
        hold off
    end
