function plot_responseSpectrum3(NDAT, E, unitstr, convf, eqname, str, rsno, nprofile, ncase, rs_bool)
...
% %         end
    ...
    elseif rsno == 2 || rsno == 3 % TEMP - No outcrop yet
        ...
        for j = 1:nprofile
            ...
            legendstr = cell(1, 1);
            for k = 3:ncase
                plot(T_range, convf(1)*squeeze(RS_x(:,ncase*j-(ncase-k),i)), plotline{k}, 'LineWidth', 1.5);
                grid on; hold on;
                xlabel('Period [s]'); ylabel(strcat('Pseudo-spectral Acceleration [',unitstr(1),']'));
                title(strcat(eqname{i},':  ',NDAT{i,j*ncase}.profile,'  -  ',str,', X'));
                legendstr{k-2} = NDAT{i,ncase*j-(ncase-k)}.case;
        ...
        for j = 1:nprofile
            ...
            legendstr = cell(1, 1);
            for k = 3:ncase
                plot(T_range, convf(1)*squeeze(RS_y(:,ncase*j-(ncase-k),i)), plotline{k}, 'LineWidth', 1.5);
                grid on; hold on;
                xlabel('Period [s]'); ylabel(strcat('Pseudo-spectral Acceleration [',unitstr(1),']'));
                title(strcat(eqname{i},':  ',NDAT{i,j*ncase}.profile,'  -  ',str,', Y'));
                legendstr{k-2} = NDAT{i,ncase*j-(ncase-k)}.case;
    ...
    elseif rsno == 4
        ...
        for j = 1:nprofile
            ...
            legendstr = cell(1, 1);
            for k = 3:ncase
                plot(T_range, squeeze(RSx(:,ncase*j-(ncase-k),i))./squeeze(RS_x(:,ncase*j-(ncase-k),i)), plotline{k}, 'LineWidth', 1.5);
                grid on; hold on;
        ...
        for j = 1:nprofile
            ...
            legendstr = cell(1, 1);
            for k = 3:ncase
                plot(T_range, squeeze(RSy(:,ncase*j-(ncase-k),i))./squeeze(RS_y(:,ncase*j-(ncase-k),i)), plotline{k}, 'LineWidth', 1.5);
                grid on; hold on;
                xlabel('Period [s]'); ylabel('Factor');
                title(strcat(eqname{i},':  ',NDAT{i,j*ncase}.profile,'  -  ',str,', Y'));
                legendstr{k-2} = NDAT{i,ncase*j-(ncase-k)}.case;
